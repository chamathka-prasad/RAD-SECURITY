          ...
          for ($o = 0; $o < count($orderManuResult); $o++) {
            ...
            if (count($quotationResult) == 1) {
              ...
              if ($manuQuotation["id"] == $quotationId) {
                ...
                $chatRoom = $db->search("SELECT * FROM `chat_room` WHERE `order_has_manifacturer_id`=?", "i", [$orderMenu["id"]]);
                if (count($chatRoom) > 0) {
                  $message->type = "error";
                  ...
                  $ohistory = $db->iud("INSERT INTO `order_history`(`date_time`,`process`,`order_id`)VALUES(?,?,?)", "ssi", [$date, 'Quotation Selected', $orderId]);
