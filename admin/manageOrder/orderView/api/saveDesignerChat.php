...
if (isset($_SESSION["rb_user"])) {
    ...
    if (empty($chatroomId)) {
    ...
    } else if (empty($chatText)) {
        $message->type = "error";
        $message->message = "Chat Text Is Empty";
        echo json_encode($message);
    } else {
        ...
        if (count($chatRoomResult) == 1) {
            ...
            $chatResult = $db->iud("INSERT INTO `chat`(`datetime`,`chat_room_id`,`type`,`message`,`is_view`,`person`)VALUES(?,?,?,?,?,?)", "sissis", [$date, $chatroom["id"], 'text', $chatText, 0, 'designer']);
...
<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; color: #333; padding: 20px;">
  ...
  <div style="max-width: 600px; margin: 0 auto; background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
    ...
    <p style="font-size: 16px;">
      You have received a new message from <strong>The Designer</strong> at <strong>' . $date . '</strong>.
    </p>
    <p style="font-size: 16px;">
...
';
                    MailSender::sendMail($manuDetails[0]["email"], "Robotic Assistance Devices-New Message From The Designer", $body);
                }
